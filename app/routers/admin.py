...
async def admin_login(request: AdminLoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Create tokens
    access_token = create_access_token(data={"sub": str(admin_user.id), "phone": admin_user.phone_number, "admin": True})
    refresh_token = create_refresh_token(data={"sub": str(admin_user.id), "phone": admin_user.phone_number, "admin": True})
...
async def verify_admin_access(admin_user: User = Depends(get_admin_user)):
    """Verify admin access - returns admin user info if valid"""
    return {
...
async def get_admin_stats(admin_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)):
...
async def delete_user(user_id: int, admin_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)):
...
async def refresh_all_caches(admin_user: User = Depends(get_admin_user), db: AsyncSession = Depends(get_db)):
    """Refresh all Redis caches (admin only)"""
    from app.redis_client import (
        invalidate_lessons_cache, invalidate_quiz_cache, 
        invalidate_grammar_topics_cache, invalidate_users_cache,
        invalidate_leaderboard_cache
    )
    
    # Invalidate all caches to force refresh
    await invalidate_lessons_cache()
    await invalidate_quiz_cache()
    await invalidate_grammar_topics_cache()
    await invalidate_users_cache()
    await invalidate_leaderboard_cache()
    
    return {"message": "All caches refreshed successfully"}
